...
namespace ElectricGuitarQuiz
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            if (userInputSelect == ConstantsVAR.START_PLAY_MODE) 
            {
                //if statement = if there is no list that within the PrintQuestionsAndAnswersForGame method display  
                UiMethods.CheckIfListIsNotEmpty(UiMethods.PrintQuestionsAndAnswersForGame());
                Logic.ValidatingCreatedQuestionList();
                // UiMethods.PrintWelcomeMessage(); // <---- this is not needed 
